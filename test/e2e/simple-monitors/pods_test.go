	...
	It("should run hook after creating a pod", func() {
		...
		assertionSteps := []struct {
			...
			},
			{
				...
				onSuccess: func() {
					fmt.Printf("Catch Synchronization run ")
				},
			},
			{
				...
				func() {
					fmt.Printf("Catch Hook executed successfully")
			...
			},
			{
				...
				func() {
					fmt.Printf("Catch Pod 'test' added")
					stopCh <- struct{}{}
				},
			},
		}
